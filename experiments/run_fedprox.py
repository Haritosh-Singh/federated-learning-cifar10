...
try:
    ...
    logger.info("Loaded FedProx config successfully.")
...
def client_fn(cid):
    try:
        ...
        logger.info(f"Initialized FedProx client {cid}")
        return FedProxClient(model, train_loaders[int(cid)], test_loader, device, mu=cfg["mu"])
    except Exception as e:
        logger.error(f"FedProx client {cid} initialization failed: {e}")
        raise
...
try:
    ...
    logger.info("FedProx simulation completed successfully.")
except Exception as e:
    logger.error(f"FedProx simulation failed: {e}")
    raise